...
export class Ball {
  ...
  constructor(startX: number, startY: number, color: string, path: number[]) {
    this.x = startX;
    this.y = startY;
    this.vx = 0;
    this.vy = 0;
  ...
  update(pins: Array<{ x: number; y: number }>, canvasHeight: number): void {
    ...
    // Deactivate if ball goes off screen
    if (this.y > canvasHeight * 1000) {
      this.isActive = false;
    }
  }
